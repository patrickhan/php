...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			if ((isset($product_info['meta_title_tag'])) && ($product_info['meta_title_tag'] != '')) {
				$this->document->title = $product_info['meta_title_tag'];
			} else {
			...
			$this->data['button_continue_shopping'] = $this->language->get('button_continue_shopping');
			...
			$this->data['youtube'] = ($this->config->get('premium') === TRUE) ? $product_info['youtube'] : '';
			...
			foreach ($results as $result) {
				...
				if($this->session->data['template'] == 'tosh' && $this->config->get('demo')){
					$this->data['products'][] = array(
						...
						'thumb' => "/image/cache/".$image."-250x250.png", 
						'price'   => $price,
						'special' => $special,
			...
			// $this->data['checkout'] = $this->url->http('checkout/somethingelse');
