...
class ControllerInformationBlog extends Controller {
  ...
  public function urls($link) {
	...
	public function index() {
		...
		$ii = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) WHERE i.location='7'");
		//echo $ii->num_rows; die;
		if (!$ii->num_rows) {
		$desc = "There are no blog currently on the site";
		}
		else
		{
		$desc = "";
		...
		$desc .= "<div id='spacerLine'><a style='color:#000; text-decoration: none; padding-bottom: 5px;' href='" . $urls . "'><h1 style='border:0'>$tt</h1></a>$dr...<br><div style='float:right'><a href='" . $urls . "'>More</a></div></div><br>";
		//if ($num=='3') {
		//echo $desc; die;
		//}
		}
		...
		if (true) {
			...
			$this->document->breadcrumbs[] = array(
				'href'      => $this->url->http('information/blog'),
				'text'      => 'Blog',
