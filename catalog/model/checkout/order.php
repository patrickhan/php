...
class ModelCheckoutOrder extends Model {
	...
	public function confirm($order_id, $order_status_id, $comment = '') {
		...
		if ($order_query->num_rows) {
			...
			$template->data['text_greeting'] = $this->config->get('config_order_message_' . (int)$order_query->row['language_id']);
			...
			foreach ($order_product_query->rows as $product) {
				...
				$gift_cerfiticate_query = $this->db->query("SELECT `gift_certificate` FROM `" . DB_PREFIX . "product` WHERE `product_id` = '" . (int)$product['product_id'] . "'");
				$gift_certificate_check = $gift_cerfiticate_query->row;
				if ($gift_certificate_check['gift_certificate'] == '1') {
					for ($gc_count = 0; $gc_count < $product['quantity']; $gc_count++) {
						$this->get_code();
						$unique = FALSE;
						do {
							$code = $this->get_code();
							$code_check = $this->db->query("SELECT `serial` FROM `gift_certificates` WHERE `serial` = '" . $code . "'");
							if ($code_check->num_rows == 0) {
								$unique = TRUE;
							}
						} while( ! $unique);
						
						$this->db->query("INSERT INTO `" . DB_PREFIX . "gift_certificates` 
							(`gift_certificate_id`, `customer_id`, `date_purchased`, `order_id`, `amount`, `serial`, `ip_used`, `order_id_used`, `date_used`) 
							VALUES (NULL, '" . $order_query->row['customer_id'] . "', NOW(), '" . $order_id . "', '" . $product['price'] . "', '" . $code . "', NULL, NULL, NULL)");
					}
				}
			}
			...
			if (isset($this->session->data['points'])) {
				$this->model_account_customer->editCustomerPoints($this->session->data['points'] * -1);
			} else if ($this->config->get('points_per_unit') > 0) {
				$this->model_account_customer->editCustomerPoints(floor($order_query->row['total']) * $this->config->get('points_per_unit'));
			}
			
			if (isset($this->session->data['gift_certificate'])) {
				$this->load->model('checkout/gift_certificate');
				$gift_certificate = array(
					'ip_used'       => $_SERVER['REMOTE_ADDR'],
					'order_id_used' => $order_id,
					'serial'        => $this->session->data['gift_certificate']
				);
				$this->model_checkout_gift_certificate->useGiftCertificate($gift_certificate);
			}
		}
	}
	...
	private function get_code($length = 8) {
		$seed = "abcdefghijklmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ234567892345678923456789";
		$str = '';
		srand((double)microtime() * 1000000);
		for ($i = 0; $i < $length; $i++) {
			$str .= substr($seed, rand() % 74, 1);
		}
		return $str;
	}
}
