...
class ModelInstall extends Model {
	public function mysql($data) {
		...
		$file = DIR_APPLICATION . 'jsicart.sql';
			...
			foreach($sql as $line) {
				...
				if (($sql != '') && (substr($tsl, 0, 2) != "--") && (substr($tsl, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$query = str_replace("DROP TABLE IF EXISTS `jsicart_", "DROP TABLE IF EXISTS `" . $data['db_prefix'], $query);
						$query = str_replace("CREATE TABLE IF NOT EXISTS `jsicart_", "CREATE TABLE `" . $data['db_prefix'], $query);
						$query = str_replace("INSERT INTO `jsicart_", "INSERT INTO `" . $data['db_prefix'], $query);
