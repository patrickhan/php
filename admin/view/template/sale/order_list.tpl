...
<?php } ?><h3>Show: <input type="radio" name="set" onClick='window.location="index.php?route=sale/order/setOn"' <?if ($alls) {?>checked='yes'<?}?> value='On'/>All <input type="radio" name="set" onClick='window.location="index.php?route=sale/order/setOff"' <?if (!$alls && !$dis) {?>checked='yes'<?}?> value='Off'/>Enabled Only <input type="radio" name="set" onClick='window.location="index.php?route=sale/order/setDis"' <?if ($dis) {?>checked='yes'<?}?> value='Dis'/>Disabled Only</h3>
<div class="box">
	<div class="left"></div>
	<div class="right"></div>
	<div class="heading">
		...
		<div id="listOrder">
		<form action="" method="post" enctype="multipart/form-data" id="list_form">
			<select name="list_order">
				<option value="*"></option>
				<option value="&sort=o.order_id&order=ASC"><?php echo $text_order_asc; ?></option>
				<option value="&sort=o.order_id&order=DESC"><?php echo $text_order_desc; ?></option>
				<option value="&amp;sort=name&amp;order=ASC"><?php echo $text_name_asc; ?></option>
				<option value="&amp;sort=name&amp;order=DESC"><?php echo $text_name_desc; ?></option>
				<option value="&amp;sort=status&amp;order=ASC"><?php echo $text_status_asc; ?></option>
				<option value="&amp;sort=status&amp;order=DESC"><?php echo $text_status_desc; ?></option>
				<option value="&amp;sort=o.date_added&amp;order=ASC"><?php echo $text_date_added_asc; ?></option>
				<option value="&amp;sort=o.date_added&amp;order=DESC"><?php echo $text_date_added_asc; ?></option>
				<option value="&amp;sort=o.total&amp;order=ASC"><?php echo $text_total_asc; ?></option>
				<option value="&amp;sort=o.total&amp;order=DESC"><?php echo $text_total_desc; ?></option>
			</select>
			<a onclick="listOrder();" class="button"><span><?php echo 'Sort'; ?></span></a>
		</form>
		</div>
		<div class="buttons"><a onclick="location='<?php echo $create; ?>'" class="button"><span><?php echo $button_create_order; ?></span></a><a onclick="$('#form').attr('action', '<?php echo $invoice; ?>'); $('#form').attr('target', '_blank'); $('#form').submit();" class="button"><span><?php echo $button_invoices; ?></span></a><a onclick="$('#form').attr('action', '<?php echo $packingslip; ?>'); $('#form').attr('target', '_blank'); $('#form').submit();" class="button"><span><?php echo $button_packingslips; ?></span></a><a onclick="$('#form').attr('action', '<?php echo $delete; ?>'); $('#form').attr('target', '_self'); $('#form').submit();" class="button"><span>Disable</span></a></div>
	</div>
	<div class="content">
...
function listOrder() {
	var list_order = $('select[name=\'list_order\']').attr('value');
