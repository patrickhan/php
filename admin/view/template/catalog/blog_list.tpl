...
<?php } ?><h3>Show: <input type="radio" name="set" onClick='window.location="index.php?route=catalog/blog/setOn"' <?if ($alls) {?>checked='yes'<?}?> value='On'/>All <input type="radio" name="set" onClick='window.location="index.php?route=catalog/blog/setOff"' <?if (!$alls && !$dis) {?>checked='yes'<?}?> value='Off'/>Enabled Only <input type="radio" name="set" onClick='window.location="index.php?route=catalog/blog/setDis"' <?if ($dis) {?>checked='yes'<?}?> value='Dis'/>Disabled Only</h3>
<div class="box">
	<div class="left"></div>
	<div class="right"></div>
	<div class="heading">
		<h1 style="background-image: url('view/image/information.png');"><?php echo $heading_title; ?></h1>
		<div class="buttons"><a class="tooltip"><?echo $qmark;?><span>Sets a URL Alias for every article that doesn't have one, making your site more search-engine friendly</span></a><a onclick="location='<?php echo $sAlias; ?>'" class="button"><span>Set Alias</span></a><a onclick="location='<?php echo $insert; ?>'" class="button"><span><?php echo $button_add_information; ?></span></a><a onclick="$('form').submit();" class="button"><span><?php echo $button_delete; ?></span></a></div>
	</div>
	<div class="content">
