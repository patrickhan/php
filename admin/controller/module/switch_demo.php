...
class Controllermoduleswitchdemo extends Controller {
	...
	public function index() {   
		$this->load->language('module/switch_demo');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			$this->model_setting_setting->editSetting('switch_demo', $this->request->post);		
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => $this->url->https('module/switch_demo'),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->https('module/switch_demo');
		...
		$this->template = 'module/switch_demo.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/switch_demo')) {
