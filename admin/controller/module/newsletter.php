		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => $this->url->https('module/newsletter'),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->https('module/newsletter');
		
		$this->data['cancel'] = $this->url->https('extension/module');

		if (isset($this->request->post['information_position'])) {
			$this->data['newsletter_position'] = $this->request->post['newsletter_position'];
		} else {
			$this->data['newsletter_position'] = $this->config->get('newsletter_position');
		}
		
		if (isset($this->request->post['information_status'])) {
			$this->data['newsletter_status'] = $this->request->post['newsletter_status'];
		} else {
			$this->data['newsletter_status'] = $this->config->get('newsletter_status');
		}
		
		if (isset($this->request->post['information_sort_order'])) {
			$this->data['newsletter_sort_order'] = $this->request->post['newsletter_sort_order'];
		} else {
			$this->data['newsletter_sort_order'] = $this->config->get('newsletter_sort_order');
		}				
