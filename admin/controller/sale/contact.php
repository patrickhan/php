...
class ControllerSaleContact extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			if (isset($this->request->post['group'])) {
				switch ($this->request->post['group']) {
					...
					case 'customer':
						...
						break;
					case 'signup':
						$results = $this->model_sale_customer->getNewsletterSignups();
			...
			if (isset($this->request->post['to']) && $this->request->post['to']) {
				foreach ($this->request->post['to'] as $customer_id) {
					if ($customer_info = $this->model_sale_customer->getCustomer($customer_id)) {
			...
			if ($emails) {
				...
				if (preg_match_all($pattern, $message, $matches)) {
					foreach ($matches[2] as $key => $value) { 
						$attachments[] = str_replace(HTTP_CATALOG, '/', str_replace('/image/', DIR_IMAGE, $value));
		...
		$this->data['text_unsubscribe'] = sprintf($this->language->get('text_unsubscribe'), HTTP_CATALOG . 'index.php?route=account/newsletter/unsubscribe', $this->config->get('config_store'));
	...
	public function customer() {
		...
		if (isset($this->request->get['keyword']) && $this->request->get['keyword']) {
			...
			$more_results = $this->model_sale_customer->getNewsletterSignupsByKeyword($this->request->get['keyword']);
			...
			foreach ($more_results as $result) {
				$customer_data[] = array(
