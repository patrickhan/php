...
class ControllerSaleCustomer extends Controller { 
	...
	private function getList() {
		...
		$this->data['export'] = $this->url->https('sale/customer/export');
	...
	public function export() {
		...
		$output = "Customer ID\t First Name\t Last Name\t E-mail\t Telephone\t Fax\t Newsletter\t Date Added\t Address 1\t Address 2\t Postcode\t City\t Zone\t Country\r\n";
		
		foreach ($customers as $customer) {
			$newsletter = ($customer['newsletter'] == '1') ? 'Yes' : 'No';
			$address = $this->model_sale_customer->getCustomerAddress($customer['customer_id'], $customer['address_id']);
		...
		header('Content-Type: text/xml, charset=ISO-8859-1; encoding=ISO-8859-1');
		echo utf8_decode($output);die;
	}
