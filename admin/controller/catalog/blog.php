...
class ControllerCatalogBlog extends Controller {
	...
	$set = $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `group` = 'setB' AND `key` = 'setB'");				
	$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `group` = 'setB', `key` = 'setB', `value` = 'On'");				
	$this->index();
	}	
	
	public function setOff() {
	$set = $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `group` = 'setB' AND `key` = 'setB'");				
	$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `group` = 'setB', `key` = 'setB', `value` = 'Off'");				
	$this->index();
	}
	
	public function setDis() {
	$set = $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `group` = 'setB' AND `key` = 'setB'");				
	$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `group` = 'setB', `key` = 'setB', `value` = 'Dis'");				
	$this->index();
	}
  ...
  PRIMARY KEY (`blogcomment_id`)
	...
	$fields = mysql_list_fields(DB_DATABASE, 'information_description');
	$columns = mysql_num_fields($fields);
	for ($i = 0; $i < $columns; $i++) {
	$field_array[] = mysql_field_name($fields, $i);
	}
	if (!in_array('keyword', $field_array)){
	$result = mysql_query('ALTER TABLE information_description ADD short TEXT');
	}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->https('catalog/blog' . $url));
		}
	...
	private function getList() {
		...
		$this->document->breadcrumbs[] = array(
			'href'      => $this->url->https('catalog/blog' . $url),
			'text'      => 'blog',
			'separator' => ' :: '
		);
		$this->data['sAlias'] = $this->url->https('catalog/blog/alias');
		$this->data['insert'] = $this->url->https('catalog/blog/insert' . $url);
		$this->data['delete'] = $this->url->https('catalog/blog/delete' . $url);
		$this->data['url'] = $this->url->https('catalog/blog' . $url);
		...
		$set = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE `group` = 'setB' AND `key` = 'setB'");
		$alls = true;
		$dis = false;
		if ($set->rows) {	
		...
		$this->data['dis'] = $dis;	
		if ($alls) {
		$information_total = $this->model_catalog_information->getTotalBlog();
		
		$results = $this->model_catalog_information->getBlog($data); 
		} else if ($dis) {
		$information_total = $this->model_catalog_information->getdTotalBlog();
		
		$results = $this->model_catalog_information->getdBlog($data); 
		} else {
		$information_total = $this->model_catalog_information->geteTotalBlog();		
		$results = $this->model_catalog_information->geteBlog($data);}
		$this->data['show'] = true;
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->https('catalog/blog/update&information_id=' . $result['information_id'] . $url)
			);
		...
		$this->data['sort_title'] = $this->url->https('catalog/blog&sort=id.title' . $url);
		$this->data['sort_sitemap'] = $this->url->https('catalog/blog&sort=i.sitemap' . $url);
		$this->data['sort_type'] = $this->url->https('catalog/blog&sort=i.type' . $url);
		$this->data['sort_location'] = $this->url->https('catalog/blog&sort=i.location' . $url);
		$this->data['sort_date_modified'] = $this->url->https('catalog/blog&sort=i.date_modified' . $url);
		$this->data['sort_sort_order'] = $this->url->https('catalog/blog&sort=i.sort_order' . $url);
		...
		$pagination->url = $this->url->https('catalog/blog' . $url . '&page=%s');
	...
	private function getForm() {
		...
		$this->document->breadcrumbs[] = array(
			'href'      => $this->url->https('catalog/blog'),
			'text'      => 'blog',
			'separator' => ' :: '
		);
		...
		if ( ! isset($this->request->get['information_id'])) {
			$this->data['action'] = $this->url->https('catalog/blog/insert' . $url);
		} else {
			$this->data['action'] = $this->url->https('catalog/blog/update&information_id=' . $this->request->get['information_id'] . $url);
		}
		
		$this->data['cancel'] = $this->url->https('catalog/blog' . $url);
...
$this->db->query("UPDATE " . DB_PREFIX . "information SET keyword = '" . $this->db->escape($keyword) . "' WHERE information_id='" . $p['information_id'] . "'");
	...
	$this->redirect($this->url->https('catalog/blog'));
	}
}
?>