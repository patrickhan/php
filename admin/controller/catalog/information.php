...
class ControllerCatalogInformation extends Controller {
	...
	$set = $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `group` = 'setI' AND `key` = 'setI'");				
	$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `group` = 'setI', `key` = 'setI', `value` = 'On'");				
	$this->index();
	}	
	
	public function setOff() {
	$set = $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `group` = 'setI' AND `key` = 'setI'");				
	$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `group` = 'setI', `key` = 'setI', `value` = 'Off'");				
	$this->index();
	}
	
	public function setDis() {
	$set = $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `group` = 'setI' AND `key` = 'setI'");				
	$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `group` = 'setI', `key` = 'setI', `value` = 'Dis'");				
	$this->index();
	}
	...
	private function getList() {
		...
		$this->data['sAlias'] = $this->url->https('catalog/information/alias');
		...
		$this->data['url'] = $this->url->https('catalog/information' . $url);
		...
		$set = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE `group` = 'setI' AND `key` = 'setI'");
		$alls = true;
		$dis = false;
		if ($set->rows) {	
		...
		$information_total = $this->model_catalog_information->getdTotalInformations();	
		} else {	
		$information_total = $this->model_catalog_information->geteTotalInformations();	
		}
		if (isset($this->request->get['type'])) {
		$type1 = $this->request->get['type'];
		}
		else
		{
		$type1 = '-1';
		...
		$results = $this->model_catalog_information->getdInformations($data);		
		} else {
		$results = $this->model_catalog_information->geteInformations($data);		
		}		
		$this->data['show'] = true;
		}
		else
		{
		if ($alls) {
		$results = $this->model_catalog_information->getInformationsType($type1);
		} else if ($dis) {
		$results = $this->model_catalog_information->getdInformationsType($type1);
		} else {		
		$results = $this->model_catalog_information->geteInformationsType($type1);
		}
		...
		$this->data['sort_sitemap'] = $this->url->https('catalog/information&sort=i.sitemap' . $url);
		$this->data['sort_type'] = $this->url->https('catalog/information&sort=i.type' . $url);
		$this->data['sort_location'] = $this->url->https('catalog/information&sort=i.location' . $url);
		$this->data['sort_date_modified'] = $this->url->https('catalog/information&sort=i.date_modified' . $url);
	...
	$ps = $this->db->query("select * from information p left join information_description pd on (p.information_id = pd.information_id) WHERE p.sitemap='1' AND p.location != '4'");
