...
final class Cart {
	public function __construct() {
		...
		$ip = $_SERVER['REMOTE_ADDR'];			$crt = $this->db->query("SELECT * FROM guest_carts WHERE `ip`='$ip'");			if ($crt->num_rows) {			$this->session->data['cart'] = unserialize($crt->row['cart']);			}
		if ( ! isset($this->session->data['cart']) || ! is_array($this->session->data['cart'])) {			$this->session->data['cart'] = array();
		}
	}
	...
	public function add($product_id, $qty = 1, $options = array()) {
		...
		}				if (!$this->customer->isLogged()) {		$this->guestUpdate();		}
	}
	...
	public function clear() {
		$this->session->data['cart'] = array();		$ip = $_SERVER['REMOTE_ADDR'];		$this->db->query("DELETE FROM guest_carts WHERE `ip`='$ip'");
	}
	...
	}	public function guestUpdate() {	$ip = $_SERVER['REMOTE_ADDR'];	$this->db->query("DELETE FROM guest_carts WHERE `ip`='$ip'");	$this->db->query("INSERT INTO guest_carts SET `ip`='$ip', date_added=now(), cart='". $this->db->escape(serialize($this->session->data['cart'])) ."'");	}
}
?>